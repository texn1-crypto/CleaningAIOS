...
После выбора вида уборки бот берёт ровно опубликованную ставку «от» из общей
таблицы страницы `/prices` и умножает её на введённую площадь. Для бизнес-центров,
офисов, торговых центров, производства и складов используется соответствующая
строка объекта; если точной строки нет, применяется явно обозначенная общая строка
вида работ. Частота не умножает ориентир автоматически, потому что сайт не определяет
прайс как цену за выезд или за месяц. Ориентир помечается как предварительный, не
является офертой и требует обследования объекта. В production мастер не запрашивает
персональные данные, пока не заполнены `COMPANY_LEGAL_NAME` и контакт политики
обработки данных.
...
фотографий проекта; происхождение выбранного файла сохраняется в metadata. SHA-256
сверяется со всей историей MediaAsset, включая отклонённые и заменённые кадры, поэтому
одна и та же фотография не выбирается повторно. Когда библиотека исчерпана, workflow
переходит в `credentials_required` и уведомляет владельца, но не подставляет старое фото.
Фраза вроде «фотографии повторяются, нужны новые» заменяет визуалы последнего
неопубликованного контент-плана, сохраняя старые как `superseded` и не запуская
публикацию.

Каждый preview содержит финальное изображение, точный текст, площадку и время. Approval
...
Системный администратор сохраняет историю всех неудачных уведомлений, но не считает
старую ошибку Telegram текущей недоступностью канала после более поздней успешной
доставки. Ошибка карточки согласования остаётся активной только пока само согласование
ожидает решения и не истекло; восстановление фиксируется отдельной записью audit log.
