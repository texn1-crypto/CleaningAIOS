...
`/mailing client@example.com second@example.com` запускает owner-only мастер.
Он фиксирует текст основания согласия, тему и тело письма, показывает preview и
создаёт защищённую задачу. Email и основание не передаются LLM; постановка писем
в очередь возможна только после отдельного owner approval.
