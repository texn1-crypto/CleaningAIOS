...
Перед разбором DOCX отдельный fail-closed preflight проверяет OPC/ZIP-контейнер:
ограничивает число элементов, размер распакованных данных и compression ratio,
запрещает шифрованные и ссылочные элементы, traversal/absolute paths,
дублирующиеся имена и неподдерживаемые алгоритмы сжатия. После проверки байты
по-прежнему не извлекаются в файловую систему, а передаются локальному parser.
