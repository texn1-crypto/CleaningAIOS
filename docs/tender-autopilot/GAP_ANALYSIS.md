...
| A4 reference scenario | PARTIAL | Reproducible structured fixture for a school order of exactly 1000 A4 packs at NMCK 1,000,000 RUB covers evidence, all hard prequalification checks, two immutable quotes, product match, economics, auction forecast, stop price, risk, checklist and approval card. A second document-path case reads stored A4 requirement bytes, performs deterministic extraction and exact-set manager review, then feeds the accepted evidence facts into a decision snapshot. Network download, PDF/OCR/table extraction, independent multi-agent review, submission/auction and post-win tail remain open |
