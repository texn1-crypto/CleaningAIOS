...
| Tender ingest из HTTP JSON feeds | PARTIAL | Общий HTTP(S) JSON contract сохраняет канонические append-only версии, revision, отличает неизменный replay и завершает каждую настроенную попытку durable receipt/outbox-событием; manager view не раскрывает query/userinfo или raw exception. Нет официальных ЕИС/ЭТП adapters, provider acknowledgements и cursor/freshness SLO |
