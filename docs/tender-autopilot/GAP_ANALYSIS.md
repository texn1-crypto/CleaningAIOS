...
| Product compliance matcher | PARTIAL | Decision snapshot хранит required/offered/match/confidence/evidence и fail-closed обязательные UNKNOWN/mismatch; локальный extractor сохраняет пары как UNKNOWN, а checksum-bound manager review переводит выбранные факты в verified/rejected. Автоматическое междокументное сопоставление и knowledge graph отсутствуют |
