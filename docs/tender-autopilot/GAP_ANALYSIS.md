...
| Product compliance matcher | PARTIAL | Decision snapshot хранит required/offered/match/confidence/evidence и fail-closed обязательные UNKNOWN/mismatch; локальный extractor сохраняет evidence-bound пары «параметр — значение» как UNKNOWN. Автоматическое сопоставление/review и knowledge graph отсутствуют |
