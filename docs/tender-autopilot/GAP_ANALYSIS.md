...
| Kill switches/capability flags | PARTIAL | Persisted owner-only global external-actions kill switch exists. Every protected Task/Orchestrator action has an explicit persisted capability flag; missing/disabled fails closed before approval, manager visibility and idempotent owner-only audited changes are implemented. Decision Engine, direct SMTP bulk-outreach delivery and direct social-provider publication now share the same global-first fail-closed gate and preserve queued work while stopped. Other direct protected entry points and per-tender controls still need centralized enforcement evidence |
