...
| Tender Intelligence | PARTIAL | The generic HTTP(S) JSON-feed boundary now keeps canonical append-only provider-item versions, distinguishes unchanged replay and emits one outbox event per real version; bearer authentication is HTTPS-only. SPb/LO scope filtering, separate participation/submission approvals and document checklist exist. The L6 foundation adds an immutable evidence/checksum-bound decision passport, Decimal economics, stop price, capital/risk gates and an idempotent approval card. Official portal adapters/cursors, immutable downloaded-document versions, supplier acquisition/RFQ and learned scoring remain unimplemented. See `docs/tender-autopilot/`. |
