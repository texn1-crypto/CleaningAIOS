...
| Approval Engine | PARTIAL | Bound approvals protect listed critical actions. Expiry, request-changes, signed Telegram decisions, immutable terminal records and exactly-once task resume are implemented. A shared global-first persisted capability gate now covers Task dispatch plus direct SMTP and social-provider worker execution; other direct protected entry points and per-resource flags remain incomplete. Read-only agent tools are separately default-deny and budgeted; a unified policy for future write-capable tools remains intentionally absent. |
