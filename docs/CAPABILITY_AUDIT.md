...
| Tender Intelligence | PARTIAL | The generic HTTP(S) JSON-feed boundary keeps canonical append-only provider-item versions, scopes external identity to source/provider, distinguishes unchanged replay and emits one outbox event per real version; bearer authentication is HTTPS-only. Downloaded evidence uses checksum-addressed byte files, append-only version metadata and fail-closed replay/conflict checks. SPb/LO scope filtering, separate participation/submission approvals and the evidence-bound decision passport remain connected. Official portal adapters/cursors, malware/archive sandbox, supplier acquisition/RFQ and learned scoring remain unimplemented. See `docs/tender-autopilot/`. |
