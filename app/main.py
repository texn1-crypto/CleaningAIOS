...
def integrations(_: Principal = Depends(principal)):
    smtp_ready = all([settings.smtp_host, settings.smtp_username, settings.smtp_password, settings.smtp_from_email])
    return {
        "postgresql": {"status": "connected" if not settings.database_url.startswith("sqlite") else "development_sqlite"},
