...
@router.get("/safety/external-actions-kill-switch")
def external_actions_kill_switch_status(
    db: Session = Depends(get_db),
    actor: Principal = Depends(principal),
):
    require_role(actor, "manager")
    return _kill_switch_view(db.get(SafetyControl, GLOBAL_EXTERNAL_ACTIONS_CONTROL))


@router.put("/safety/external-actions-kill-switch")
def update_external_actions_kill_switch(
    ...
    if payload.active and len(reason) < 3:
        raise HTTPException(422, "A reason is required when activating the kill switch")
    row = db.get(SafetyControl, GLOBAL_EXTERNAL_ACTIONS_CONTROL)
    ...
    if changed:
        ...
        audit(
            db,
            actor.subject,
            "safety.external_actions_kill_switch_updated",
            "safety_control",
            row.key,
            details,
        )
        event_bus.publish(
            db,
            "safety.kill_switch_updated",
            "safety_control",
            row.key,
            details,
            idempotency_key=f"safety-control:{row.key}:version:{row.version}",
            actor=actor.subject,
        )
    db.commit()
    db.refresh(row)
...
def list_tender_supplier_quote_snapshots(
    ...
    if not tender or tender.record_type != "tender":
        raise HTTPException(404, "Tender not found")
    rows = db.scalars(
        select(TenderSupplierQuoteSnapshot)
        .where(TenderSupplierQuoteSnapshot.record_id == record_id)
        .order_by(TenderSupplierQuoteSnapshot.id.desc())
    ).all()
    return [supplier_quote_snapshot_view(row) for row in rows]


@router.post(
    "/tenders/{record_id}/supplier-quote-snapshots",
    status_code=201,
)
def create_tender_supplier_quote_snapshot(
    record_id: int,
    ...
    if tender.status in TERMINAL_TENDER_STATUSES:
        raise HTTPException(409, f"Tender is in terminal status: {tender.status}")
    try:
        snapshot, created = persist_supplier_quote_snapshot(
            db,
            tender,
            payload,
            actor=actor.subject,
    ...
    tender.data = {
        **(tender.data or {}),
        "latest_supplier_quote_snapshot_id": snapshot.id,
        "latest_supplier_quote_snapshot_hash": snapshot.input_hash,
        "latest_supplier_quote_status": snapshot.status,
    }
    event_bus.publish(
        db,
        "tender.supplier_quote_recorded",
        "tender",
        str(record_id),
        {
            "snapshot_id": snapshot.id,
            ...
            "hard_stops": result["hard_stops"],
            "verification_gaps": result["verification_gaps"],
            "economics_allowed": result["economics_allowed"],
            "created": created,
        },
        idempotency_key=f"tender-supplier-quote:{record_id}:{snapshot.input_hash}",
        actor=actor.subject,
    )
    audit(
        db,
        actor.subject,
        "tender.supplier_quote_recorded",
        "tender_supplier_quote_snapshot",
        str(snapshot.id),
        {
            "tender_id": record_id,
            "input_hash": snapshot.input_hash,
    ...
    db.commit()
    db.refresh(snapshot)
    return {**supplier_quote_snapshot_view(snapshot), "created": created}
...
@router.post("/tender-documents/{document_id}/requirements/review")
def review_document_requirements(
    document_id: int,
    payload: TenderRequirementReviewCreate,
    db: Session = Depends(get_db),
    actor: Principal = Depends(principal),
):
    require_role(actor, "manager")
    ...
    if not tender or tender.record_type != "tender":
        raise HTTPException(409, "Tender document is not bound to a tender")
    try:
        result, created = review_requirement_candidates(
            document,
    ...
    event_bus.publish(
        ...
        str(document.record_id),
        {
            ...
            "unknown_compliance_count": result["unknown_compliance_count"],
            "created": created,
        },
        idempotency_key=(
            f"tender-requirements-review:{document.id}:{result['review_hash']}"
        ),
        actor=actor.subject,
    )
    audit(
